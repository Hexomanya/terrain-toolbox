...
{	
    override void Run()
    {
		...
		for (int i; i < entityCount; i++)
		{
			...
			entitySource.Get(DAB_Names.FLAGS, oldFlags);
			bool isRelativeSwitchedOn = oldFlags & EntityFlags.RELATIVE_Y;
			
			if(isRelativeSwitchedOn) {
				oldFlags &= ~EntityFlags.RELATIVE_Y;
				placementMode = DAB_Names.NONE;
			} else {
				oldFlags |= EntityFlags.RELATIVE_Y;
				placementMode = DAB_Names.SLOPEANDCONTACT;
			}
			
			//Placement
			worldEditorAPI.SetVariableValue(entitySource, null, DAB_Names.PLACEMENT, placementMode.ToString());

			//Flags
			worldEditorAPI.SetVariableValue(entitySource, null, DAB_Names.FLAGS, oldFlags.ToString());
		}
